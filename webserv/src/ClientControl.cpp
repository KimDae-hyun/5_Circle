...
void		ClientControl::saveFile(void)
{
	...
	for (int idx = 0; idx < (int)multipart.size(); idx++)
	{
		...
		if(extension != "jpg" && extension != "png" && extension != "jpeg"
			...
			continue ;
		file.open("/Users/daekim/Desktop/eval/webserv/state_pages/" + multipart[idx].file_name, std::ios::out);
...
int ClientControl::checkUri(void)
{
	...
	body_size = it->getClientBodySize();
...
void ClientControl::deleteFile()
{
	string path;
	struct stat st;
	string path_info = "/Users/daekim/Desktop/eval/webserv/state_pages/delete.html";
